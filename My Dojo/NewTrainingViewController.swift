...
class NewTrainingViewController: UIViewController {
    ...
    @IBAction func addNewtrainingToProgress(_ sender: UIButton) {
        DatabaseManager.context.perform { [unowned self] in
            guard let training: Training = Training.createTraining(at: self.dateTextField.text!,
                                                                   journal: self.journalTextField.text!,
                                                                   inManagedObjectContext: DatabaseManager.context) else { return }
            do {
...
extension NewTrainingViewController : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: Constants.newTrainingTechniqueCell,
                                                       for: indexPath) as? NewTrainingTechniqueTableViewCell
            else { fatalError("Cell is not of kind NewTrainingTechniqueTableViewCell") }
...
extension NewTrainingViewController: ListensForNotifications {
    ...
    internal func addNotificationObservers() {
        ...
        let trainingCreatedObserver: NSObjectProtocol? = notificationCenter.addObserver(forName: .newTrainingCreatedNotification,
                                                                                        object: nil,
