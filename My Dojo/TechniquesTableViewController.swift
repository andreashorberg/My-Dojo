...
class TechniquesTableViewController: CoreDataTableViewController {
    ...
    fileprivate func updateUI() {
        if let context = managedObjectContext {
            ...
            if isSelectionMode {
                let key = "japaneseName"
                let selector = #selector(NSString.localizedCaseInsensitiveCompare(_:))
                request.sortDescriptors = [NSSortDescriptor(key: key, ascending: true, selector: selector)]
                
                fetchedResultsController = NSFetchedResultsController<NSManagedObject>(fetchRequest: request,
                                                                                       managedObjectContext: context,
                                                                                       sectionNameKeyPath: nil,
                                                                                       cacheName: nil)
            } else {
                let key1 = "chapter.japaneseName"
                let key2 = "japaneseName"
                let selector = #selector(NSString.localizedCaseInsensitiveCompare(_:))
                
                request.sortDescriptors = [NSSortDescriptor(key: key1, ascending: true, selector: nil),
                                           NSSortDescriptor(key: key2, ascending: true, selector: selector)]
                
                fetchedResultsController = NSFetchedResultsController<NSManagedObject>(fetchRequest: request,
                                                                                       managedObjectContext: context,
                                                                                       sectionNameKeyPath: key1,
                                                                                       cacheName: nil)
            }
...
//            numberOfSelectedTechniques = selectedTechniques.count
    ...
    fileprivate var isTechniqueSelected: Bool {
        get { return !selectedTechniques.isEmpty }
        set {
    ...
    func removeSelection() {
        let title = "Remove techniques"
        let message = "This action will remove all your selected techniques, are you sure?"
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let cell = tableView.cellForRow(at: indexPath), isSelectionMode {
            ...
            if isSearchActive {
                selectedTechnique = filteredTechniques[indexPath.row]
            } else if let technique = fetchedResultsController?.object(at: indexPath) as? Technique {
...
//                numberOfSelectedTechniques += 1
    ...
    override func tableView(_ tableView: UITableView, didDeselectRowAt indexPath: IndexPath) {
        if let cell = tableView.cellForRow(at: indexPath), isSelectionMode {
            ...
            let deselectedTechnique: Technique
            if isSearchActive {
                deselectedTechnique = filteredTechniques[indexPath.row]
            } else if let technique = fetchedResultsController?.object(at: indexPath) as? Technique {
                deselectedTechnique = technique
            } else {
                return
            }
